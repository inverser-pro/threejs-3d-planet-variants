  ...
  "remembered": {
    ...
    "Animated": {
      "0": {
        "seeThrough": false,
        "thickness": 0.02,
        "backgroundHex": "#fdf1cc",
        "fillHex": "#c6d6b8",
        "strokeHex": "#987f69",
        "dashEnabled": true,
        "dashAnimate": true,
        "dashRepeats": 6,
        "dashLength": 0.6000000000000001,
        "dashOverlap": true,
        "noiseA": false,
        "noiseB": false,
        "insideAltColor": false,
    ...
    "FunZone": {
      "0": {
        "seeThrough": true,
        "thickness": 0.02,
        "backgroundHex": "#dad6ca",
        "fillHex": "#1bb0ce",
        "strokeHex": "#4f8699",
        "dashEnabled": false,
        "dashAnimate": false,
        "dashRepeats": 2,
        "dashLength": 0.7000000000000001,
        "dashOverlap": true,
        "noiseA": true,
        "noiseB": true,
        "insideAltColor": true,
...
//\
        ...
        meshCircles = new THREE.Mesh(geometryCircles, shaderMaterial);
...
/*
    ...
    for (let j = 0; j < Math.PI; j += (2 * Math.PI) / 100) {
        pointsa.push(Math.cos(j), Math.sin(j), 0);
    }
    const linea = new MeshLine.MeshLine();
    linea.setPoints([new THREE.Vector3(0,0,0),new THREE.Vector3(2,2,2),new THREE.Vector3(3,3,3)]);

    //linea.setPoints(geometry, p => 2); // makes width 2 * lineWidth
    //linea.setPoints(geometry, p => 1 - p); // makes width taper
    //linea.setPoints(geometry, p => 2 + Math.sin(50 * p)); // makes width sinusoidal

    const materiala = new MeshLine.MeshLineMaterial({color:0xff0000,side:THREE.DoubleSide,wireframe:true});
    const mesha = new THREE.Mesh(linea, materiala);
