...
const sketch = ({ context }) => {
    ...
    obj.img.onload=()=>{
        ...
        obj.ar=[];
        for(var y = 0; y < obj.w; y++) {
            for(let x = 0; x < obj.w; x++) {
                let r = obj.data[((obj.w * y) + x) * 4];
                let g = obj.data[((obj.w * y) + x) * 4 + 1];
                let b = obj.data[((obj.w * y) + x) * 4 + 2];
                let a = obj.data[((obj.w * y) + x) * 4 + 3];
                //console.log(r,g,b,a)
                if(a>0)obj.ar.push([x-obj.w/10,y-obj.w/10])
            }
            //if(y>1000)break;
        }
        ...
        //obj.ar=shuffle(fillUp(obj.ar, 1500))
        obj.ar.forEach((el,index)=>{

            const geometry = new THREE.CircleBufferGeometry(0.006,4);
            lonHelper.rotation.y = THREE.MathUtils.degToRad(el[0]) + lonFudge;
            latHelper.rotation.x = THREE.MathUtils.degToRad(el[1]) + latFudge;
