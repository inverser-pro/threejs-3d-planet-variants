...
const MeshLine = require('three.meshline/src/THREE.MeshLine.js')
...
/*
    ...
    const pointsp = curvep.getPoints( 50 );
    const geometryp = new THREE.BufferGeometry().setFromPoints( pointsp );

    const materialp = new THREE.LineBasicMaterial( { color : 0xff0000 } );

// Create the final object to add to the scene
    const curveObjectp = new THREE.Line( geometryp, materialp );
...
// set opacity to 50%
    ...
    //         circleLocation2.position.set(1.8,-1,0);
    // // aLight.position.set(0,-3,3);
    //
...
*/
    ...
    // scene.add(light2)
        // anime({targets:light2.position,x:[-6,5],loop:true,duration:2000,easing:'linear'})
    ...
    //createCurve({q:[.8,.55,-.2139],w:[.7,.85,-.27],e:[.652,.765,-.27]});
    //hong
    ...
    class CurveNew{
        constructor(stop){
            ...
            this.material=new THREE.LineBasicMaterial({color:0x381D6D});
    ...
    function setNewCurve(){
        ...
        anime({
            targets: newCurve.rotation, z: [-9, 10],loop:true, duration: 10000, easing: 'linear'/*, complete: () => {
                // const i = newCurve.uuid;
...
//
    function createCurve(q){
        ...
        //const geometryCurve = new THREE.PlaneBufferGeometry(1,1,1,1);

        //console.log(possss)
        const materialCurve = new THREE.LineBasicMaterial( { color : 0xffffff } );

        /*const materialCurve=new THREE.ShaderMaterial({
            extension:{
  ...
  vUv = uv;
            ...
            fragmentShader:`uniform float time;
                ...
                void main(){
                    ...
                    gl_FragColor=vec4(vUv.x,o,0.0,1.);
                }`
        })
*/
        // Create the final object to add to the scene
...
const fragmentShader=`
        varying float vAlpha;
        void main() {
        gl_FragColor = vec4( .3,.3,.9, vAlpha );
    }`;
*/
    /*Planet geometry*/
    const obj={};
...
*/
        ...
        anime({
            targets:meshCircles.scale,x:[0,1],y:[0,1],z:[0,1],duration:3600,easing:'linear'
        })
        //console.log(meshCircles)
        // let uq=0,inttrt=null
        // inttrt=setInterval(()=>{
        //     console.log(uq)
        //     if(uq<=meshCircles.length){
        //         anime({
        //             targets:meshCircles[uq].scale,loop:true,x:[0,1],y:[0,1],z:[0,1],duration:600,easing:'linear'
        //         })
        //         uq++
        //     }else{
        //         clearInterval(inttrt)
        //     }
        // },500)
...
/*
        const shaderMaterial = new THREE.ShaderMaterial( {
            uniforms:{color: { value: new THREE.Color( 0xffff00 ) }},
            vertexShader,
            fragmentShader,
            side:THREE.BackSide,
            transparent:  true
...
/*
            ...
            const alphas = new Float32Array( numVertices  ); // 1 values per vertex
            for( var i = 0; i < numVertices; i ++ ) {
                // set alpha randomly
                alphas[ i ] = getRandomFloat(0,.5);
                // console.log(getRandomFloat(0,.5))
            }
            mesh.geometry.setAttribute( 'alpha', new THREE.BufferAttribute( alphas, 1 ) );*/
            // console.log( mesh.geometry.getAttribute('alpha'))
            scene.add(mesh);
            parent.add(mesh);
        ...
        //anime({targets:circlesAnimatedAr[0].material,opacity:[0,1],loop:true,duration:3000,easeng:'linear',direction:'alternate'})

        // circlesAnimatedAr.map(e=>{
        //     anime({targets:e.material,opacity:[0,1],loop:true,duration:1000,easeng:'linear',direction:'alternate',deelay:1000})
        // })
//animation main icosah points cilinder material
        ...
        // setInterval(()=>{
        //     isf++;
        //     // if(isf<circlesAnimatedAr.length&&circlesAnimatedAr[isf]){
        //         anime({targets:circlesAnimatedAr[isf].material,opacity:[0,1],loop:true,duration:1000,easeng:'linear',direction:'alternate'})
        //     // }else{
        //     //     return
        //     // }
        // },2000)

        // for(let i=0;i<=circlesAnimatedAr.length;i++){
...
/*
    ...
    var geometry3 = new THREE.TubeGeometry( path0, 20, 2, 8, false );

// to buffer goemetry
    geometry3 = new THREE.BufferGeometry().fromGeometry( geometry3 );
    nMax = geometry3.attributes.position.count;
...
/*
    ...
    const path3 = new CustomSinCurve();
    const geometry3 = new THREE.TubeGeometry( path3, 32, 1, 2, false );
...
let time=0;
  function render() {
    ...
    //  if(time<5) console.log(circlesAnimatedAr.length)
    /*if(circlesAnimatedAr&&time%10==0){
        // console.log(circlesAnimatedAr)
        const rand=Math.round(getRandomFloat(0,circlesAnimatedAr.length))
        //circlesAnimatedAr.map(e=>{
             const alphas = circlesAnimatedAr[0].geometry.attributes.alpha;
         if(time<150)console.log(alphas)
      ...
      /*if(loaded) {
          geometry.vertices.forEach(function (particle, index) {
              var dX, dY, dZ;
              dX = Math.sin(i + index / 6) / 10;
              dY = Math.cos(i + index / 6) / 10;
              dZ = 0;
