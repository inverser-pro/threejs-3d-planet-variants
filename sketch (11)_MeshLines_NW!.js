...
*/
   renderer.render( scene, camera );
    ...
    class DashedCurve extends THREE.Mesh {
        constructor({
                        size = SIZE,
                        ampl = AMPL_MAX,
            ...
            points.push(new THREE.Vector3(-segmentLength * NBR_OF_CURVES, 0, 0));
            console.log(points)
            // create geometry
...
/*
    const curve = new THREE.SplineCurve(
        [new THREE.Vector3(0,0,0),new THREE.Vector3(1,2,1), new THREE.Vector3(3,3,3)]
    );
    const path = new THREE.Path(curve.getPoints(100));
    const geometryt = path.createPointsGeometry(100);
    const line = new MeshLine.MeshLine();
    line.setGeometry(geometryt, p => {
        const a = ((0.5 - Math.abs(0.5 - p)) * 5);
        return a;
    });
    // create material
    // create mesh line
    const materialt = new MeshLine.MeshLineMaterial({
 ...
 */
    ...
    const jjj=new THREE.Mesh(line.geometryt, materialt);scene.add(jjj)
*/
//    const lineM = new MeshLine.MeshLine();
   /* const geometryM = new THREE.Geometry();
    for (let j = 0; j < Math.PI; j += 2 * Math.PI / 100) {
        const v = new THREE.Vector3(Math.cos(j), Math.sin(j), 0);
        geometryM.vertices.push(v);
    }*/
/*    lineM.setPoints([0,1,2]);
    const materialM = new MeshLine.MeshLineMaterial({color:0xff0000});
    const meshM = new THREE.Mesh(lineM, materialM);
    scene.add(meshM);
*/
    //let i=0;
  function render() {
