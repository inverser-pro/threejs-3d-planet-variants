...
const sketch = ({ context }) => {
    ...
    function addMapInf(posCil1,posCir2,rotateCir=null,main=false){
    ...
    //Btc (main)
    let c1=addMapInf([.65,.918,-.27],[.652,.765,-.27],[1.25,-.62,-1],true);
    //Lux
    let c2=addMapInf([.62,.89,-.13],[.62,.81,-.13],[1.45,-.62,-.5]);
    //London
    let c3=addMapInf([.422,1,.152],[.422,.92,.152],[1.7,-.72,0]);
    //Malta
    let c4=addMapInf([.81,.63,-.2139],[.81,.55,-.2139],[1.2,-1,0]);
    //usa
    let c5=addMapInf([.2139,.85,.6921],'',[1.2,-1,0]);
    //usa2
    let c6=addMapInf([-.2139,.85,.6921],'',[1.2,-1,0]);
    //hong
    let c7=addMapInf([-.3461,.5,-.906],[.81,.55,-.2139],[1.2,-1,0]);
    //sing
    let c8=addMapInf([-.6921,-.1,-.7738],'',[1.2,-1,0]);
    //btc animate circle
    let c9=addMapInf([.65,.918,-.27],[.652,.765,-.27],[1.25,-.62,-1],true);

    anime.timeline()
        .add({
            targets:c1[0].scale,x:[0,1],y:[0,1],z:[0,1],duration:1000,deelay:100,easing:'linear'
        }).add({
            targets:c1[1].scale,x:[0,1],y:[0,1],z:[0,1],duration:1000,easing:'linear'
        }).add({
            targets:c2[0].scale,x:[0,1],y:[0,1],z:[0,1],duration:1000,deelay:100,easing:'linear'
        }).add({
            targets:c2[1].scale,x:[0,1],y:[0,1],z:[0,1],duration:1000,easing:'linear'
        }).add({
            targets:c3[0].scale,x:[0,1],y:[0,1],z:[0,1],duration:1000,deelay:100,easing:'linear'
        }).add({
            targets:c3[1].scale,x:[0,1],y:[0,1],z:[0,1],duration:1000,easing:'linear'
        }).add({
            targets:c4[0].scale,x:[0,1],y:[0,1],z:[0,1],duration:1000,deelay:100,easing:'linear'
        }).add({
            targets:c4[1].scale,x:[0,1],y:[0,1],z:[0,1],duration:1000,easing:'linear'
        }).add({
            targets:c5[0].scale,x:[0,1],y:[0,1],z:[0,1],duration:1000,deelay:100,easing:'linear'
        }).add({
            targets:c6[0].scale,x:[0,1],y:[0,1],z:[0,1],duration:1000,deelay:100,easing:'linear'
        }).add({
            targets:c7[0].scale,x:[0,1],y:[0,1],z:[0,1],duration:1000,deelay:100,easing:'linear'
        }).add({
            targets:c7[1].scale,x:[0,1],y:[0,1],z:[0,1],duration:1000,deelay:1000,easing:'linear'
        }).add({
            targets:c8[0].scale,x:[0,1],y:[0,1],z:[0,1],duration:1000,deelay:1000,easing:'linear'
        })
...
*/
    ...
    fontLoader.load('font-roboto.json',(font)=>{
        ...
        let txt9=createText('Golang',[.22,.9,.68],[0,.4,0],.03,font,0x8a8a8a);
        let txt10=createText('USA',[.22,.84,.68],[0,.4,0],.03,font,0x84B3DF);

        let txt11=createText('Future',[-.2,.9,.69],[0,-.4,0],.03,font,0x8a8a8a);
        let txt12=createText('USA',[-.2,.84,.69],[0,-.4,0],.03,font,0x84B3DF);

        let txt13=createText('Freedom',[-.368,.55,-.9],[0,3.6,0],.03,font,0x8a8a8a);
        let txt14=createText('Hong Kong',[-.368,.48,-.9],[0,3.6,0],.03,font,0x84B3DF);

        let txt15=createText('Labtob',[-.71,-.04,-.77],[0,3.8,0],.03,font,0x8a8a8a);
        let txt16=createText('Singapore',[-.71,-.1,-.77],[0,3.8,0],.03,font,0x84B3DF);
        ...
        anime.timeline()
            ...
            .add({
                targets:txt2.scale,x:[0,1],y:[0,1],z:[0,1],duration:600,easing:'linear'
            })
            .add({
            ...
            .add({
                targets:txt10.scale,x:[0,1],y:[0,1],z:[0,1],duration:600,easing:'linear',complete:()=>{
                    //usa
                    addNewParticle({q:[.2139,.7738,.6921],w:[.7,1.2,.8],e:[.2,1.5,.4],r:[.652,.765,-.27]},20)
                }
            })
            .add({
            ...
            .add({
                targets:txt15.scale,x:[0,1],y:[0,1],z:[0,1],duration:600,easing:'linear',delay:30000
            })
            .add({
                targets:txt16.scale,x:[0,1],y:[0,1],z:[0,1],duration:600,easing:'linear',complete:()=>{
                    //singapug
                    addNewParticle({q:[-.6921,-.2139,-.7738],w:[1,1.2,-1.5],e:[1,1.2,-1.7],r:[.652,.765,-.27]},20)
                }
            }).add({
                targets:txt13.scale,x:[0,1],y:[0,1],z:[0,1],duration:600,easing:'linear'
            })
            .add({
                targets:txt14.scale,x:[0,1],y:[0,1],z:[0,1],duration:600,easing:'linear',complete:()=>{
                    //hong
                    addNewParticle({q:[-.3461,.4278,-.906],w:[.6,1,-1.5],e:[.7,.8,-1],r:[.652,.765,-.27]},20)
                }
            })
...
/*
  ...
  function addBoxes(file) {
    ...
    data.map((row, latNdx) => {
        ...
        const geometry = new THREE.CircleBufferGeometry(0.006,4);
        ...
        //if(mmm<10){mmm++;console.log(geometry)}
...
function Animation(startPosition, control0Range, control1Range, endPosition,prefabCount=30) {
    ...
    const prefabGeometry = new THREE.PlaneGeometry(.009,.009);
    //var  = 30;
