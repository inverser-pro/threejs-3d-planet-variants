...
const sketch = ({ context }) => {
    ...
    obj.img.onload=()=>{
        ...
        for (let i=0;i<obj.ar.length;i=i+2){
            obj.counter++;
            //-89 ... 0 ... 87
            //if(true/*obj.ar[i+1]!==undefined&&obj.prewW-obj.ar[i+1][1]<0*/){
            //if(obj.counter<1000&&obj.ar[i-1]!==undefined)console.log(obj.ar[i][0]-obj.ar[i-1][0]==1,obj.counter);
            //if(obj.ar[i-1]!==undefined&&Math.abs(Math.round(obj.ar[i-1][1]))%2==1){
            if(
                obj.ar[i-1]!==undefined
                &&Math.abs(Math.round(obj.ar[i-1][1]))%2==1
                //&&Math.abs(obj.currentX-obj.ar[i-1][0])>20
            ){
                //obj.currentY=obj.ar[i][1];//-89
                //obj.currentX=obj.ar[i][0];//-89
               // if(
               ///     obj.ar[i][0]-obj.ar[i-1][0]==1
                //    &&obj.ar[i+1]!==undefined
                //) {
                    obj.nAr.push(obj.ar[i])
                //}
            }
                //if(obj.ar[i-1]!=undefined&&obj.currentY!==obj.ar[i-1][1]);
            /*}else{
                //-51.06451612903225
                obj.prewW=obj.ar[i][1];
            }*/
        };
        /*for (let i = 0; i < obj.nAr.length; i = i+4) {
            obj.nAr.push(obj.nAr[i]);
        };
        for (let i = 0; i < obj.nAr.length; i = i+2) {
            obj.nAr.push(obj.nAr[i]);
        };*/
        //obj.prewY,obj.prewwYY=null;
        obj.counter2=obj.counter3=0;
        obj.nAr.forEach(e=>{
            obj.counter2++;
            obj.counter3++;
            //i++;
            ...
            /*const q=*/lonHelper.rotation.y = THREE.MathUtils.degToRad(e[0]) + lonFudge;
            const w=latHelper.rotation.x = THREE.MathUtils.degToRad(e[1]) + latFudge;
            //console.log(w-obj.prewLatX)
            /*if(e[1]<0){//-47.546973457 - -46.3452663
                if(e[1]-obj.prewY==0){obj.coef=true}
            }else{//1.021211 - 2.0121465
                if(obj.prewY-e[1]==0){obj.coef=true}

            }*/
            //if(i<1000){
                //console.log(Math.abs(obj.prewY)-Math.abs(e[1]),obj.prewwYY)
            //}

            originHelper.updateWorldMatrix(true, false);
            geometry.applyMatrix4(originHelper.matrixWorld);
            if(w-obj.prewLatX===0&&obj.counter2%3==0) {
                geometries.push(geometry);
            }else{
                if(obj.counter3>1000&&obj.counter2%2==0)geometries.push(geometry);
            }
            /*obj.prewY=e[1];
            obj.prewQ=q
            obj.prewW=w*/
